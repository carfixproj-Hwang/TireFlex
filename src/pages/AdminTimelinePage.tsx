...
function isoAtKst(dateStr: string, hhmm: string) {
  ...
  // KST 기준을 명시하기 위해 +09:00 포함
  return `${dateStr}T${hhmm}:00+09:00`;
}

function inRange(slotIso: string, startIso: string, endIso: string) {
...
export default function AdminTimelinePage() {
  ...
  const rowForSlot = useMemo(() => {
    return slots.map((s) => {
      ...
      const res = reservations.find((r) => overlaps(s.iso, 30, r.scheduled_at, addMinutes(new Date(r.scheduled_at), r.duration_minutes).toISOString()));
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button onClick={refresh} disabled={loading} style={{ padding: "8px 12px", cursor: "pointer" }}>
          새로고침
        </button>
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <div style={{ fontWeight: 600 }}>{badgeText}</div>
                {isReserved ? (
      ...
      <div style={{ marginTop: 12, fontSize: 12, opacity: 0.75 }}>
        운영시간: 09:00-18:00 (KST), 30분 슬롯 기준
      </div>
    </div>
  );
}
